...
@st.cache_data
def load_data() -> Tuple[Dict[str, Any], Dict[str, Any], Dict[str, Any], Dict[str, Any]]:
    """Load all YAML data files with error handling.
    
    Returns:
        Tuple of (capabilities, risks, controls, baseline) dictionaries
    """
    ...
    try:
        with open(os.path.join(data_dir, 'baseline.yaml'), 'r') as f:
            baseline = yaml.safe_load(f)
        if not baseline:
            st.error("Failed to load baseline data. Please check data/baseline.yaml")
            return capabilities, risks, controls, {}
    except FileNotFoundError:
        st.error("Baseline file not found. Please ensure data/baseline.yaml exists")
        return capabilities, risks, controls, {}
    except yaml.YAMLError as e:
        st.error(f"Error parsing baseline.yaml: {str(e)}")
        return capabilities, risks, controls, {}
    except Exception as e:
        st.error(f"Unexpected error loading baseline: {str(e)}")
        return capabilities, risks, controls, {}
    
    return capabilities, risks, controls, baseline
